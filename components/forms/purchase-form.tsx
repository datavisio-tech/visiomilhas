  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data?.success) {
        setOperation(data);
        setMessage(
          `Compra operacional concluída. Saldo anterior ${formatPoints(data.previousBalance)} → saldo atual ${formatPoints(data.newBalance)}.`,
        );
        form.reset();
        router.refresh();
        try {
          ...
          if (accountId) {
            const [accountResponse, replayResponse] = await Promise.all([
            ...
            if (accountJson?.success) setInspection(accountJson.inspection);
    ...
    >
      ...
      {operation && (
        <section className="space-y-3 rounded border bg-gray-50 p-3">
          <div className="font-semibold">Replay de compra</div>
          <div className="grid gap-2 text-sm md:grid-cols-3">
            <NarrativeBox
              title="ANTES"
              lines={[
                `Saldo anterior: ${formatPoints(operation.previousBalance)}`,
                `CPM anterior: ${formatMoneyCents(operation.previousCpmCents)}`,
              ]}
            />
            <NarrativeBox
              title="AÇÃO"
              lines={[
                `Compra de ${formatPoints(operation.pointsBought)}`,
                `Custo total: ${formatMoneyCents(operation.totalCostCents)}`,
              ]}
            />
            <NarrativeBox
              title="DEPOIS"
              lines={[
                `Saldo atualizado: ${formatPoints(operation.newBalance)}`,
                `Lote criado: ${formatPoints(operation.pointsBought)}`,
                `CPM atualizado: ${formatMoneyCents(operation.newCpmCents)}`,
              ]}
            />
          </div>
        </section>
