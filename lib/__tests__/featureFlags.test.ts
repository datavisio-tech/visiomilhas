...
describe("featureFlags (stateless)", () => {
  ...
  it("returns false for empty string, '0', 'false' and invalid values", () => {
    expect(
      isFifoMovementsEngineEnabled({ USE_FIFO_MOVEMENTS_ENGINE: "" }),
    ).toBe(false);
    expect(
      isFifoMovementsEngineEnabled({ USE_FIFO_MOVEMENTS_ENGINE: "0" }),
    ).toBe(false);
    expect(
      isFifoMovementsEngineEnabled({ USE_FIFO_MOVEMENTS_ENGINE: "false" }),
    ).toBe(false);
    expect(
      isFifoMovementsEngineEnabled({ USE_FIFO_MOVEMENTS_ENGINE: "nope" }),
    ).toBe(false);
  });
