...
import type { MileEntryRecord, MilePointLotRecord, TransferRecord, PointLot } from "../services/movements";
...
export function createDrizzleMovementsRepo(db = appDb()) {
  async function findAccountById(accountId: number) {
    const rows = await db.select().from(program_accounts).where(eq(program_accounts.id, accountId)).limit(1);
    const r = (rows as any[])[0];
    return r ? { id: r.id } : null;
  }

  async function insertEntry(entry: MileEntryRecord) {
    const res = await db.insert(mile_entries).values({
  ...
  async function insertLot(lot: MilePointLotRecord) {
    const res = await db.insert(mile_point_lots).values({
  ...
  async function updateLotRemaining(lotId: number, remaining: number) {
    await db.update(mile_point_lots).set({ remainingPoints: remaining }).where(eq(mile_point_lots.id, lotId));
  }

  async function updateProgramAccountBalance(accountId: number, deltaPoints: number) {
    ...
    await db.update(program_accounts).set({ currentPointsBalance: next }).where(eq(program_accounts.id, accountId));
  }

  async function getAvailableLots(accountId: number): Promise<PointLot[]> {
    const rows = await db.select().from(mile_point_lots).where(eq(mile_point_lots.accountId, accountId));
  ...
  async function insertTransfer(transfer: TransferRecord) {
    const res = await db.insert(mile_transfers).values({
